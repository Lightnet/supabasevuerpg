<script setup>
// https://vuejs.org/api/built-in-special-attributes.html#is
// https://vuejs.org/guide/components/async.html#basic-usage
// https://stackoverflow.com/questions/67669820/how-to-define-component-name-in-vue3-setup-tag
// https://sfc.vuejs.org/#
// 
// https://medium.com/@miladmeidanshahi/vue3-import-and-register-components-in-composition-api-4cc36555fe19
// 
// 
import { shallowRef, ref, getCurrentInstance } from 'vue';

import HomeBase from "./ui/ViewHomeBase.vue";
import Character from "./ui/ViewCharacter.vue";
import Inventory from "./ui/ViewInventory.vue";
import Map from "./ui/ViewMap.vue";
import Battle from "./ui/ViewBattle.vue";
import GameSettings from "./ui/ViewGameSettings.vue";

//const ctx = getCurrentInstance();
//console.log(ctx)
//console.log(HomeBase)
//const view = ref('HomeBase');
const view = shallowRef(HomeBase);

const tabs = [
  {name:'Home Base',comp:HomeBase},
  //{name:'Home Base',comp:'HomeBase'},
  //{name:'Home Base',comp:'vue:home-base'},
  //{name:'Character',comp:'character'},
  {name:'Character',comp:Character},
  //{name:'Inventory',comp:'Inventory'},
  {name:'Inventory',comp:Inventory},
  //{name:'Map',comp:'Map'},
  {name:'Map',comp:Map},
  //{name:'Battle',comp:'Battle'},
  {name:'Battle',comp:Battle},
  //{name:'Settings',comp:'Settings'}  
  {name:'Settings',comp:GameSettings}  
]

</script>
<template>
  <div>
    <template v-for="tab in tabs">
    <a href="#" @click="view = tab.comp">{{tab.name}}</a> |  
    </template>
  </div>
  <div>
    <component :is="view"></component>
  </div>
</template>